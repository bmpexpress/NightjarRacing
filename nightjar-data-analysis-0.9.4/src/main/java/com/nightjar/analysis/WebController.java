...
public class WebController {
 ...
 @PostMapping("/file-data/{source}")ResponseEntity<?>fileData(@PathVariable String source,@RequestBody Models.FilterRequest r,HttpSession s){if(!authorised(s))return unauthorised();if(!source.equalsIgnoreCase("event")&&!source.equalsIgnoreCase("test"))return ResponseEntity.badRequest().body(Map.of("error","Source must be event or test"));return ResponseEntity.ok(Map.of("source",source,"rows",store.auxiliaryData(source,r)));}
