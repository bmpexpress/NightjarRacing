...
public class WebController {
 private final DataStore store; private final AuxiliaryDataStore auxiliary; private final String passwordHash;
 public WebController(DataStore store,AuxiliaryDataStore auxiliary){this.store=store;this.auxiliary=auxiliary;String hash=System.getenv().getOrDefault("NIGHTJAR_APP_PASSWORD_SHA256","").trim().toLowerCase(Locale.ROOT);String plain=System.getenv().getOrDefault("NIGHTJAR_APP_PASSWORD","");passwordHash=hash.isBlank()?(plain.isBlank()?"":sha256(plain)):hash;}
 record LoginRequest(String password){}
 record AuxiliaryFilter(List<String>eventDates,String fromDate,String toDate,String startTime,String endTime,boolean trimByTime){}
 ...
 @GetMapping("/state")ResponseEntity<?>state(HttpSession s){if(!authorised(s))return unauthorised();Map<String,Object>out=new LinkedHashMap<>(store.state());out.put("auxiliaryCounts",auxiliary.counts());return ResponseEntity.ok(out);}
 @PostMapping(value="/upload",consumes=MediaType.MULTIPART_FORM_DATA_VALUE)ResponseEntity<?>upload(HttpSession s,@RequestPart(value="log",required=false)MultipartFile log,@RequestPart(value="polar",required=false)MultipartFile polar,@RequestPart(value="events",required=false)MultipartFile events,@RequestPart(value="testData",required=false)MultipartFile testData,@RequestPart(value="eventList",required=false)MultipartFile eventList,@RequestPart(value="sailChart",required=false)MultipartFile sail,@RequestPart(value="debrief",required=false)MultipartFile debrief){if(!authorised(s))return unauthorised();store.upload(log,polar,events,eventList,sail,debrief);auxiliary.upload(events,testData);Map<String,Object>out=new LinkedHashMap<>(store.state());out.put("auxiliaryCounts",auxiliary.counts());return ResponseEntity.ok(out);}
 ...
 @PostMapping("/auxiliary/{source}")ResponseEntity<?>auxiliary(@PathVariable String source,@RequestBody AuxiliaryFilter r,HttpSession s){if(!authorised(s))return unauthorised();if(!source.equalsIgnoreCase("event")&&!source.equalsIgnoreCase("test"))return ResponseEntity.badRequest().body(Map.of("error","Source must be event or test"));return ResponseEntity.ok(Map.of("rows",auxiliary.filtered(source,r.eventDates(),r.fromDate(),r.toDate(),r.startTime(),r.endTime(),r.trimByTime())));}
