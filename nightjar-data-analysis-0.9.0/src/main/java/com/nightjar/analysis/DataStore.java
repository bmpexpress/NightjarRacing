...
public class DataStore {
 ...
 private boolean plotDownsamplingEnabled=false;
 ...
 public Models.DataResponse filtered(Models.FilterRequest req,boolean forPlot){lock.readLock().lock();try{
  ...
  List<String>warnings=new ArrayList<>(loadWarnings);Map<LocalDate,LocalDateTime> guns=gunCutoffs(eventDays,warnings);ArrayList<Models.Row> matches=new ArrayList<>();
  for(Models.Row row:rows){LocalDateTime ts=row.timestamp;if(ts==null)continue;LocalDate day=ts.toLocalDate();if(!eventDays.isEmpty()&&!eventDays.contains(day))continue;if(eventDays.isEmpty()&&((from!=null&&day.isBefore(from))||(to!=null&&day.isAfter(to))))continue;LocalDateTime cut=guns.get(day);if(cut!=null&&ts.isBefore(cut))continue;if(req.trimByTime()&&(ts.toLocalTime().isBefore(start)||ts.toLocalTime().isAfter(end)))continue;matches.add(row);}
  int matched=matches.size();boolean sampled=forPlot&&plotDownsamplingEnabled&&matched>maxPlotPoints;List<Models.Row> selected=sampled?evenSample(matches,maxPlotPoints):matches;String tsCol=mapping.get("timestamp");
  List<Map<String,Object>> result=new ArrayList<>(selected.size());for(Models.Row row:selected)result.add(row.toMap(visible,tsCol));
  return new Models.DataResponse(result,warnings,rows.size(),matched,result.size(),sampled,new LinkedHashMap<>(mapping));
 }finally{lock.readLock().unlock();}}
 private static List<Models.Row> evenSample(List<Models.Row> input,int limit){if(input.size()<=limit)return input;ArrayList<Models.Row> out=new ArrayList<>(limit);if(limit==1){out.add(input.get(0));return out;}double step=(input.size()-1.0)/(limit-1.0);int previous=-1;for(int i=0;i<limit;i++){int index=(int)Math.round(i*step);if(index<=previous)index=Math.min(input.size()-1,previous+1);out.add(input.get(index));previous=index;}return out;}
